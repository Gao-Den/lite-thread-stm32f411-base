...
void nrf_link_fsm_idle(lt_msg_t* msg) {
	...
	case NRF_LINK_PURE_MSG_OUT: {
		...
		if (fifo_is_empty(&nrf_pdu_fifo)) {
			...
			if_pure_msg.header.type = msg->msg_type;
	...
	case NRF_LINK_COMMON_MSG_OUT: {
		...
		if (fifo_is_empty(&nrf_pdu_fifo)) {
			...
			if_common_msg.header.type = ((lt_common_msg_t*)msg)->msg_header.msg_type;
	...
	case NRF_LINK_RECV_MSG: {
		...
		NRF_LINK_LOG("[nrf_link] pdu_id free after used for received msg: %d\n", pdu_id);
		break;
	}
